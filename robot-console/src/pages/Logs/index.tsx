...
const buildLogsWebsocketUrl = (
  ...
  resolvedBase.search = "";
  resolvedBase.searchParams.set("selector", selector);
  if (limit) {
...
export const LogPage = () => {
  ...
  const connect = useCallback(() => {
    if (!selector) {
      setStatus("error");
      setStatusMessage("Provide a selector before tailing logs");
      return;
    }
    disconnect();
    setLogs([]);
            ...
            {logs.length === 0 ? (
              ...
              logs.map((entry) => (
                <div className="log-entry" key={entry.id}>
                  ...
                  {entry.labels && (
                    <span className="labels">{formatLabels(entry.labels)}</span>
                  )}
                </div>
              ))
            )}

...
def _build_loki_tail_url(
    ...
    selector: LogQL stream selector(s), e.g. '{service="polyflow-webrtc.service"}'
              or list of selectors to OR together.
    """

    # Normalize selector(s) to list because tail expects repeated match[]
    ...
    params: dict[str, object] = {
        "match[]": matchers,   # <-- IMPORTANT: tail uses match[]
    }
...
def create_app() -> FastAPI:
    ...
    async def tail_logs(websocket: WebSocket):
        selector = websocket.query_params.get("selector")
        if not selector:
            await websocket.close(code=1002, reason="selector query parameter required")
            return
        ...
        upstream_url = _build_loki_tail_url(selector, limit, start, delay_for)
